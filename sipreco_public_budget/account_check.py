...
class account_check(models.Model):
    ...
    def check_check_cancellation(self):
        for check in self:
            if check.type == 'issue' and check.state not in [
                    'draft', 'to_be_handed', 'handed']:
                raise Warning(_(
                    'You can not cancel issue checks in states other than\
                    "draft", "to be handed" or "handed".\
                    First try to change check state.'))
            # third checks received
            elif check.type == 'third' and (
                    not self.third_handed_voucher_id) and (
                    check.state not in ['draft', 'holding']):
                raise Warning(_(
                    'You can not cancel received third checks in states other\
                    than "draft or "holding".\
                    First try to change check state.'))
            # third checks handed
            elif check.type == 'third' and (
                    self.third_handed_voucher_id) and (
                    check.state not in ['draft', 'holding']):
                raise Warning(_(
