...
class AccountMove(models.Model):
    ...
    def _recompute_to_pay_amount_when_with_advance_payment(self):
        ...
        """
        for rec in self:
            if rec.payment_state in ['paid', 'in_payment'] and not rec._get_reconciled_payments():
    ...
    def _get_to_pay_amount_to_date(self):
        ...
        if self.payment_state in ['paid', 'in_payment'] and not self._get_reconciled_payments():
        ...
        # "mandado a pagar": órdenes de pago confirmadas o en ejecución, salientes
        # o entrantes. Una nota de crédito cancelada con una orden entrante (el
        # proveedor devolvió plata) tiene que descontar del importe a pagar, igual
        # que descuenta del devengado y del pagado. Solo se excluyen las rechazadas.
        to_pay_states = {'confirmed', 'signature_process', 'signed', 'in_process', 'paid'}
        if to_date:
            lines = lines.filtered(lambda x: any(
                pg.sipreco_state in to_pay_states
                and pg.confirmation_date and pg.confirmation_date <= to_date
                for pg in x.payment_ids))
        else:
            lines = lines.filtered(lambda x: any(
                pg.sipreco_state in to_pay_states for pg in x.payment_ids))
    ...
    def _get_paid_amount_to_date(self):
        ...
        if to_date:
            ...
            lines = lines.filtered(lambda x: any(
                pg.state in ['in_process', 'paid'] and pg.date <= to_date for pg in x.payment_ids))
        else:
            lines = lines.filtered(lambda x: any(
                pg.state in ['in_process', 'paid'] for pg in x.payment_ids))
