...
class BudgetTransaction(models.Model):
    ...
    def _compute_advance_to_return_amount(self):
        ...
        for rec in self:
            rec.advance_to_return_amount = (
                rec.preventive_amount - rec.invoiced_amount - rec.advance_returned_amount)
    ...
    @api.depends(
        ...
        'advance_payment_ids.approval_state',
    )
    ...
    def mass_payment_group_create(self):
        ...
        for invoice in self.invoice_ids.filtered(
            ...
            already_paying = self.payment_ids.filtered(
                lambda x: x.state != 'canceled').mapped('to_pay_move_line_ids')
